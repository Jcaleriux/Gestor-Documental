  ...
  const actions = [
    {
      ...
      onClick: nota.ruta_xml
        ? () => openProtectedInNewTab(`/api/files/xml?path=${encodeURIComponent(nota.ruta_xml)}`)
        : null,
    ...
    },
    {
      ...
      onClick: nota.ruta_xml || nota.ruta_pdf
        ? () => openProtectedInNewTab(`/api/notas-credito/${nota.id}/manifest`)
        : null,
    ...
    <div className="factura-actions" data-factura-menu="true">
      {nota.ruta_pdf ? (
        <button
          ...
          type="button"
          onClick={() => openProtectedInNewTab(`/api/files/pdf?path=${encodeURIComponent(nota.ruta_pdf)}`)}
        >
