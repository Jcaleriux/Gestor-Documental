...
const rechazoTesoreria = handleRequest((req) => {
  ...
  return useCases.rechazoTesoreria({ id, facturaId, motivo, usuario });
...
const accionTesoreria = handleRequest((req) => {
  ...
  return useCases.accionTesoreria({ id, facturaId, accion, destino, motivo, usuario });
...
const crearTramite = handleRequest((req) => {
  ...
  return useCases.crearTramite({ sociedad_id, factura_ids, retencion_factura_ids, usuario });
...
const cambiarEstado = handleRequest((req) => {
  ...
  return useCases.cambiarEstado({ id, estado, usuario, motivo, force, pagos_documentos });
...
const decisionDocumento = handleRequest((req) => {
  ...
  return useCases.decisionDocumento({ id, facturaId, etapa, decision, motivo, usuario });
